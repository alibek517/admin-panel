...
const Receipt = React.forwardRef(({ order, commissionPercent, formatPrice }, ref) => {
  const restaurantName = useSelector((state) => state.restaurant.restaurantName);

  if (!order) return null;

  // Table number ni to'g'ri olish
  const tableNumber = order.table?.number || order.carrierNumber || "N/A";
  const isDelivery = tableNumber?.startsWith("+") || tableNumber === "N/A";

  // Total with commission ni hisoblash
  const totalPrice = order.orderItems?.reduce(
    (sum, item) => sum + parseFloat(item.product?.price || 0) * (item.count || 0),
    0
  ) || 0;
  const totalWithCommission = totalPrice + totalPrice * (commissionPercent / 100 || 0);
  ...
  return (
    <div>
      ...
      <div className="receipt" ref={ref}>
        ...
        <p style={{ fontSize: "12px" }}>
          <strong>{isDelivery ? "Telefon" : "Stol"}:</strong> {tableNumber}
        </p>
        <p style={{ fontSize: "12px" }}>
          <strong>Sana:</strong>{" "}
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style={{ width: "20%", fontSize: "9px" }}>Narxi</th>
                <th style={{ width: "30%", fontSize: "9px" }}>Summasi</th>
              </tr>
            </thead>
            <tbody>
              {order.orderItems?.map((item, index) => (
                <tr key={index}>
                  ...
                  <td style={{ fontSize: "8px" }}>{formatPrice(item.product?.price || 0)}</td>
                  <td style={{ fontSize: "8px" }}>{formatPrice((item.product?.price || 0) * item.count)}</td>
                </tr>
              ))}
            </tbody>
          </table>
