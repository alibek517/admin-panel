  ...
  export default function Zakazlar() {
        ...
        try {
          ...
          setState((prev) => ({
            ...
            tables: prev.tables.map((table) =>
              table.id === tableId
                ? { ...table, status: statusMapToFrontend[backendStatus] || status }
                : table
            ),
          }));
          console.log(`Table ${tableId} status updated successfully to ${status}`);
        } catch (error) {
          const message = handleApiError(
            error,
          ...
          const handleCloseAndPrint = useCallback(
            async (order) => {
              ...
              try {
                ...
                // Agar buyurtma stolga tegishli bo'lsa, stolni bo'sh qilish
                if (order.tableId) {
    ...
    const handleSaveTableChange = useCallback(async () => {
      ...
      try {
        ...
        console.error(`Failed to change table for order ${state.selectedOrderId}:`, message);
        setState((prev) => ({
          ...prev,
          error: message,
          isSaving: false,
    ...
    const handleRemoveItem = useCallback(
      ...
      [state.editingOrder, state.isSaving, calculateTotalPrice, token]
    );
              ...
              <div className="order-list">
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => {
                    ...
                    const commission = calculatedTotal * (state.commissionPercent / 100);
                      ...
                      >
                        <div className="order-card__header">
                          <div className="order-card__info">
                            ...
                            >
                              <strong>
                                {order.table?.number ? order.table?.name : "Доставка"} -{" "}
                              </strong>
                              <strong>
                                {order.table?.number || order.carrierNumber || "Йўқ"}
                              </strong>
                              {order.user && (
                                <span
                                  style={{
                          ...
                          <div className="order-card__actions">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                ...
                                e.stopPropagation();
                                console.log(
                                  "Toggle button clicked for order:",
                                  order.id
                                );
            ...
            order={
              state.currentOrder
                ? deepClone({
                    ...
                    tableNumber: state.currentOrder.table?.number || state.currentOrder.carrierNumber || "",
